...
**Speaker:** 
"Hello, my name is Sadhana Adhikari, and welcome to my comprehensive video demonstration for the Information Retrieval module ST7071CEM. 

In this video, I will be walking you through my full-stack web application, which was built to solve two primary challenges: 
...
**[Visuals: Terminal window showing `scheduler.py` or `custom_scraper.py` running. Show the console output as pages are parsed. Then switch to MongoDB Atlas to show the raw data documents.]**

**Speaker:**
"Let's begin with Task 1: The Vertical Search Engine. A search engine is only as good as its data, so the first step was building a reliable web crawler. 
...
**[Visuals: Hit enter. Show the results populating on the screen. Click through some of the pagination or scroll through the results.]**
...
**[Visuals: Show the Task 2 frontend interface where the user triggers or views the clustering results. Show graphs or clustered data if available.]**
...
**[Visuals: Briefly scroll through `rss_collector.py` and `app.py` in VS Code, highlighting the modular functions and comments. Then return to the React Frontend homepage for the final sign-off.]**
